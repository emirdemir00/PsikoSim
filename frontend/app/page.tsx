    ...
    <main className="flex h-screen bg-[#F8FAFC] text-[#1E293B] font-sans overflow-hidden antialiased">
      ...
      {showProfileModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#0F172A]/40 backdrop-blur-sm p-4">
          <div className="bg-white p-6 md:p-8 rounded-[32px] shadow-2xl max-w-sm w-full relative">
            ...
            <h2 className="text-xl md:text-2xl font-bold text-[#2B3674] mb-2">Hoş Geldiniz!</h2>
            <p className="text-xs text-[#A3AED0] font-bold mb-6">Profilinizi kişiselleştirmek için bilgilerinizi girebilirsiniz.</p>
            <div className="space-y-4 mb-8">
              <input value={tempName} onChange={e => setTempName(e.target.value)} placeholder="Adınız Soyadınız" className="w-full bg-[#F8F9FB] border border-slate-200 p-3 md:p-4 rounded-xl text-sm font-bold focus:outline-none focus:ring-2 focus:ring-[#3E34FA] text-[#2B3674]" />
      ...
      {activePage === 'chat-session' ? (
         <div className="flex w-full h-full bg-white relative">
            ...
            {chatPopup && (
                <div className="absolute inset-0 bg-white/95 z-40 flex flex-col p-6 md:p-12 animate-in fade-in">
                    <div className="flex justify-between items-center mb-8 border-b pb-4">
                        <h2 className="text-2xl md:text-3xl font-bold text-[#2B3674]">
                    ...
                    <div className="flex-1 overflow-y-auto text-base md:text-lg text-slate-600">
                        {chatPopup === 'dosya' && (
                            <div className="space-y-4">
                                <h3 className="text-lg md:text-xl font-bold text-[#3E34FA]">Vaka Özeti</h3>
                        ...
                        {chatPopup === 'metrik' && (
                            <div className="space-y-6">
                                <div className="bg-[#F8F9FB] p-6 rounded-2xl border border-slate-100">
                                   ...
                                   <p className="text-2xl md:text-3xl font-bold text-[#2B3674]">{mesajlar.length} Mesaj</p>
                                </div>
                                <div className="bg-[#F8F9FB] p-6 rounded-2xl border border-slate-100">
                                   <p className="text-sm font-bold text-slate-400 uppercase mb-2">Mevcut Durum</p>
                                   <p className="text-lg md:text-xl font-bold text-[#3E34FA]">{mesajlar.length > 5 ? "Tansiyon Artıyor" : "Gözlem Aşaması"}</p>
                                </div>
                            </div>
                        )}
                        {chatPopup === 'mudahale' && (
            ...
            {/* Sol Menü (Desktopta görünür, Mobilde gizli) */}
            <aside className="hidden lg:flex w-[280px] bg-[#F8F9FB] border-r border-slate-200 flex-col p-6 shrink-0 z-10">
            ...
            <section className="flex-1 flex flex-col bg-[#F8FAFC] relative border-r border-slate-200">
               ...
               <header className="h-16 md:h-20 px-4 md:px-8 flex items-center justify-between border-b border-slate-100 bg-white/50 backdrop-blur-md z-10 shrink-0">
                  <div className="flex items-center gap-2 bg-[#E9E3FF] px-3 md:px-4 py-1.5 rounded-full">
                  ...
                  <div className="max-w-3xl mx-auto space-y-4 md:space-y-6">
                     {mesajlar.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-64 text-center opacity-60 mt-10 md:mt-20">
                           ...
                           <h3 className="text-base md:text-lg font-bold text-[#2B3674] mb-2">{isTestMode ? 'Test Başlıyor...' : 'Danışan odada bekliyor.'}</h3>
                           <p className="text-xs md:text-sm text-[#A3AED0] px-4">{isTestMode ? 'Promptunuzu güncelleyip test edebilirsiniz.' : 'İlk sorunuzu yazabilir veya sesli iletebilirsiniz.'}</p>
                        </div>
                     ) : (
                        mesajlar.map((m, i) => (
                           <div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
                              <div className={`p-4 md:p-6 rounded-[20px] md:rounded-[24px] text-[14px] md:text-[15px] leading-relaxed shadow-sm max-w-[90%] md:max-w-[85%] ${
                                 m.role === 'user' 
                              ...
                              <p className="text-[9px] md:text-[10px] font-bold text-[#A3AED0] uppercase mt-2 tracking-widest">
               ...
               <div className="p-4 md:p-8 z-10 relative">
                  <div className="max-w-3xl mx-auto relative bg-white rounded-full shadow-xl shadow-slate-200 flex items-center p-1.5 md:p-2 border border-slate-100">
                     <button onClick={toggleRecording} className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center mr-1 md:mr-2 transition-all ${isRecording ? 'bg-red-500 text-white animate-pulse' : 'bg-indigo-50 text-[#3E34FA] hover:bg-indigo-100'}`}>
                        🎙️
                     </button>
                     <input 
                        value={input}
                        ...
                        placeholder={isRecording ? "Dinleniyor..." : "Mesajınızı yazın..."}
                        disabled={isRecording}
                        className="flex-1 bg-transparent border-none py-3 px-2 md:px-4 text-[14px] md:text-[15px] font-medium text-slate-700 focus:outline-none placeholder:text-slate-400 disabled:opacity-50"
                     />
                     <button onClick={() => mesajIlet(input, false)} disabled={isRecording} className="w-10 h-10 md:w-12 md:h-12 bg-[#2111E7] text-white rounded-full flex items-center justify-center shadow-md hover:bg-[#1A0CB0] transition-colors disabled:opacity-50">
            ...
            {/* Sağ Menü (Desktopta görünür, Mobilde gizli) */}
            {isTestMode ? (
               <aside className="hidden xl:flex w-[360px] bg-[#F8F9FB] p-6 flex-col gap-4 overflow-y-auto shrink-0 z-10 border-l border-slate-200">
      ...
      <>
        ...
        <aside className={`fixed lg:static inset-y-0 left-0 transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-300 ease-in-out w-[260px] bg-[#F8FAFC] border-r border-slate-200 flex flex-col shrink-0 z-50`}>
          <div className="flex items-center justify-between p-6 lg:p-8">
            ...
            <button onClick={() => setIsMobileMenuOpen(false)} className="lg:hidden p-2 text-slate-400 hover:text-slate-600 rounded-lg">
               ✕
            </button>
          </div>
          ...
          <nav className="flex-1 flex flex-col px-4 space-y-1 mt-2">
            ...
            <div className="mt-auto pt-6 lg:pt-10 pb-2">
          ...
          <div className="p-4 lg:p-6 space-y-4">
            ...
            <button className="w-full flex items-center justify-center lg:justify-start gap-3 px-4 py-3 text-[14px] font-semibold text-[#64748B] hover:text-[#1E293B] transition-all">
           ...
           <div className="flex-1 overflow-y-auto">
                 ...
                 <div className="flex-1 p-4 md:p-8 space-y-6 md:space-y-8">
                   <div className="bg-gradient-to-r from-[#2111E7] to-[#4537F3] rounded-[24px] md:rounded-[32px] p-8 md:p-12 text-white shadow-xl relative overflow-hidden">
                     <div className="relative z-10 max-w-2xl">
                       <h1 className="text-2xl md:text-4xl font-bold mb-3 md:mb-4 leading-snug">Psikolog Adayları İçin Geliştirilmiş Sanal Danışan Simülasyonu</h1>
                       <p className="text-indigo-100 text-sm md:text-base mb-6 md:mb-8 opacity-90">Teorik bilginizi pratikle buluşturun, güvenli bir ortamda gerçek vaka deneyimleri yaşayın.</p>
                       <div className="flex gap-4">
                          <button onClick={() => setActivePage('library')} className="w-full md:w-auto bg-white text-[#3E34FA] px-6 md:px-8 py-3.5 rounded-full font-bold text-sm shadow-lg flex items-center justify-center gap-2 hover:scale-105 transition-all">▶ Vaka Seç ve Başla</button>
                       </div>
                     </div>
                   </div>
                   ...
                   <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                     {[
                       ...
                       <div key={i} className={`bg-white p-5 md:p-6 rounded-[20px] md:rounded-[24px] shadow-sm border-l-4 ${card.bc}`}>
                         ...
                         <p className="text-xl md:text-2xl font-bold text-[#2B3674] mb-2">{card.v}</p>
                   ...
                   <div className="bg-white rounded-[24px] md:rounded-[32px] p-6 md:p-8 shadow-sm border border-slate-100 relative overflow-hidden group transition-all hover:shadow-md">
                     <div className="flex justify-between items-center mb-6">
                       <div>
                         <h3 className="text-base md:text-lg font-bold text-[#1E293B]">Aktif Simülasyon</h3>
                     ...
                     <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-8 bg-[#F8FAFC] p-4 md:p-6 rounded-2xl md:rounded-3xl border border-slate-50">
                       ...
                       <div className="flex-1 w-full">
                         <div className="flex items-center gap-3 mb-1">
                           <h4 className="text-sm md:text-base font-bold text-[#1E293B]">{currentVaka?.vaka_adi || "Seçili Vaka Yok"}</h4>
                         ...
                         <p className="text-[11px] md:text-xs text-[#64748B] font-medium">Son etkileşim: {mesajlar.length > 0 ? mesajlar[mesajlar.length - 1].time : "Henüz başlanmadı"}</p>
                       </div>
                       <div className="flex items-center gap-6 md:gap-10 sm:pr-4 w-full sm:w-auto justify-between sm:justify-end border-t sm:border-t-0 pt-4 sm:pt-0">
                         <div className="text-center">
                           <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest">MESAJLAR</p>
                           <p className="text-base md:text-lg font-bold text-[#1E293B]">{mesajlar.length}</p>
                         </div>
                         <div className="text-center">
                           <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest">SÜRE</p>
                           <p className="text-base md:text-lg font-bold text-[#1E293B]">{formatTime(seansSuresi)}</p>
                         </div>
                         <button 
                           onClick={() => { if(currentVaka) setActivePage('chat-session') }}
                           className="w-10 h-10 bg-white border border-slate-200 rounded-full flex items-center justify-center shadow-sm hover:bg-[#3E34FA] hover:text-white transition-all sm:group-hover:scale-110"
                         >
                           ➔
                         </button>
                       </div>
             ...
             {activePage === 'library' && (
               <div className="p-4 md:p-8 space-y-6 md:space-y-8 animate-in fade-in duration-300">
                  <div className="max-w-3xl">
                     <h1 className="text-2xl md:text-3xl font-bold text-[#2B3674] mb-3 md:mb-4">Simülasyon Vakaları</h1>
                     <p className="text-sm md:text-base text-[#A3AED0] font-medium leading-relaxed">Klinik deneyiminizi geliştirmek için titizlikle hazırlanmış gerçekçi vaka senaryolarını keşfedin.</p>
                  </div>
             ...
             {activePage === 'team' && (
               <div className="p-4 md:p-12 space-y-8 md:space-y-12 animate-in fade-in duration-500 max-w-7xl mx-auto">
                 <div className="max-w-3xl">
                   <h1 className="text-2xl md:text-4xl font-bold text-[#1E293B] mb-3 md:mb-4">Ekip Üyeleri</h1>
                   <p className="text-sm md:text-lg text-[#64748B] font-medium leading-relaxed">Psiko-Sim Laboratuvarı, teknolojiyi ve psikolojiyi bir araya getirerek yarının klinik eğitimini bugün inşa ediyor. Empati odaklı yaklaşımımızla dijital simülasyonların sınırlarını zorluyoruz.</p>
                 </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                   {[
                     ...
                     <div key={i} className="bg-white rounded-[24px] md:rounded-[32px] p-6 md:p-8 border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 group relative overflow-hidden flex flex-col">
                       ...
                       <div className="flex flex-col items-center text-center mb-6">
                          <div className="w-24 h-24 md:w-32 md:h-32 bg-slate-100 rounded-[24px] md:rounded-[32px] mb-4 overflow-hidden shadow-inner border-4 border-white">
                             <img 
                                src={u.img} 
                                alt={u.n} 
                          ...
                          <h3 className="text-lg md:text-xl font-bold text-[#1E293B]">{u.n}</h3>
                          <p className="text-[10px] md:text-[11px] font-bold text-[#3E34FA] uppercase tracking-widest mt-1">{u.r}</p>
                       </div>

                       <div className="space-y-4 flex-1 flex flex-col">
                          <div>
                            <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">GÖREV</p>
                            <p className="text-xs md:text-sm font-bold text-slate-700 leading-tight">{u.g}</p>
                          </div>
                          <p className="text-xs md:text-sm text-slate-500 leading-relaxed flex-1">{u.d}</p>
                          
                          <a href={u.l} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 w-full py-3 bg-slate-50 text-[#3E34FA] rounded-xl md:rounded-2xl font-bold text-sm hover:bg-[#3E34FA] hover:text-white transition-all">
                   ...
                   {/* EKİBE KATILIN KARTI */}
                   <div className="border-2 border-dashed border-slate-200 rounded-[24px] md:rounded-[32px] p-6 md:p-8 flex flex-col items-center justify-center text-center space-y-4 hover:border-[#3E34FA] transition-colors cursor-pointer group">
                      <div className="w-14 h-14 md:w-16 md:h-16 bg-slate-50 rounded-full flex items-center justify-center text-xl md:text-2xl group-hover:scale-110 transition-transform">✨</div>
                      <h3 className="text-base md:text-lg font-bold text-[#1E293B]">Ekibe Katılın</h3>
                      <p className="text-xs text-slate-500 font-medium px-2 md:px-4">Psikoloji ve teknolojinin kesişiminde fark yaratmak ister misiniz?</p>
                      <button className="text-[#3E34FA] font-bold text-xs md:text-sm hover:underline">Açık Pozisyonları İncele</button>
                   </div>
                 </div>

                 {/* VİZYONUMUZ KARTI */}
                 <div className="bg-white rounded-[24px] md:rounded-[40px] p-6 md:p-10 border border-slate-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12 mt-8 md:mt-12">
                    <div className="max-w-2xl text-center md:text-left">
                       <h3 className="text-xl md:text-2xl font-bold text-[#1E293B] mb-3 md:mb-4">Vizyonumuz</h3>
                       <p className="text-sm md:text-base text-slate-600 leading-relaxed font-medium">Psiko-Sim Laboratuvarı ekibi olarak, her klinik vakada insan hikayesini merkeze alıyoruz.</p>
                    </div>
                    <div className="flex gap-3 md:gap-4 w-full md:w-auto justify-center">
                       <div className="bg-[#F8FAFC] p-4 md:p-6 rounded-2xl md:rounded-3xl text-center flex-1 md:min-w-[120px] border border-slate-50">
                          <p className="text-2xl md:text-3xl font-black text-[#3E34FA]">5+</p>
                          <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase mt-1">Yıllık Ar-Ge</p>
                       </div>
                       <div className="bg-[#F8FAFC] p-4 md:p-6 rounded-2xl md:rounded-3xl text-center flex-1 md:min-w-[120px] border border-slate-50">
                          <p className="text-2xl md:text-3xl font-black text-[#3E34FA]">12k</p>
                          <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase mt-1">Simülasyon</p>
                       </div>
                    </div>
                 </div>
               </div>
             ...
             {activePage === 'about' && (
               <div className="p-4 md:p-12 space-y-6 md:space-y-8 animate-in fade-in duration-300">
                  <div className="bg-[#3E34FA] rounded-[24px] md:rounded-[32px] p-8 md:p-16 text-white flex justify-between items-center shadow-xl relative overflow-hidden">
                     <div className="relative z-10">
                        <h1 className="text-3xl md:text-5xl font-bold mb-3 md:mb-4">Proje Hakkında</h1>
                        <p className="text-sm md:text-xl text-indigo-100 font-medium max-w-2xl">Psiko-Sim, klinik psikoloji eğitimini dijital bir laboratuvara dönüştüren, empatik analizi merkeze alan bir simülasyon platformudur.</p>
                     </div>
                     <div className="text-[100px] md:text-[160px] absolute right-0 md:right-10 top-1/2 -translate-y-1/2 opacity-10 md:opacity-20 pointer-events-none">⚙️</div>
                  </div>
                  <div className="flex flex-col gap-6 md:gap-8">
                     <div className="flex-1 bg-white rounded-[24px] md:rounded-[32px] p-6 md:p-12 border-l-[6px] md:border-l-[8px] border-[#3E34FA] shadow-sm">
                        <div className="flex items-center gap-3 md:gap-4 mb-6 md:mb-8">
                           <div className="w-10 h-10 md:w-12 md:h-12 bg-red-50 text-red-500 rounded-xl flex items-center justify-center text-lg md:text-xl font-black shrink-0">!</div>
                           <h2 className="text-xl md:text-3xl font-bold text-[#2B3674]">Önemli Not</h2>
                        </div>
                        <p className="text-sm md:text-base text-slate-700 font-bold leading-relaxed mb-4 md:mb-6">Bu proje psikolog adaylarının kendilerini geliştirmeleri ve alıştırma yapmaları adına yapılmıştır. Vaka içerikleri eğitim ve simülasyon amacıyla hazırlanmıştır.</p>
                        <div className="bg-[#F8F9FB] p-5 md:p-8 rounded-xl md:rounded-2xl border border-slate-100 mb-4 md:mb-8">
                           <p className="text-xs md:text-sm text-slate-500 font-medium italic">Her bireyin yaşantısı, psikolojik yapısı ve terapi sürecine verdiği tepkiler farklıdır. Burada sunulan vakalar genellenebilir kesin doğrular değildir.</p>
                        </div>
                        <p className="text-xs md:text-sm text-slate-600 font-medium leading-relaxed mb-4 md:mb-8">Uygulama içeriği, kullanıcıların klinik karar verme süreçlerinin yerini almaz. Tanı koyma ve tedavi süreçleri yalnızca yetkin ruh sağlığı profesyonelleri tarafından yürütülmelidir.</p>
                     </div>
                  </div>
               </div>
             )}
                ...
                <div className="flex-1 flex flex-col h-full animate-in fade-in duration-300">
                   {!isAdminAuth ? (
                      <div className="flex-1 flex items-center justify-center p-4 md:p-8">
                         <div className="bg-white p-8 md:p-12 rounded-[24px] md:rounded-[32px] shadow-xl max-w-md w-full border border-slate-100 text-center relative overflow-hidden">
                            <div className="absolute top-0 left-0 w-full h-2 bg-[#3E34FA]"></div>
                            <div className="w-16 h-16 md:w-20 md:h-20 bg-[#F4F7FE] text-[#3E34FA] rounded-[20px] md:rounded-[24px] flex items-center justify-center text-2xl md:text-3xl mx-auto mb-6 shadow-inner">🔒</div>
                            <h2 className="text-xl md:text-2xl font-bold text-[#2B3674] mb-2">Yetkili Girişi</h2>
                            <p className="text-xs md:text-sm text-[#A3AED0] font-bold mb-6 md:mb-8">Vaka kütüphanesini yönetmek için uzman şifresini girin.</p>
                            <div className="space-y-4 md:space-y-6">
                               <input type="password" value={adminPassword} onChange={(e) => setAdminPassword(e.target.value)} placeholder="Şifre (admin)" className="w-full bg-[#F8F9FB] border border-slate-200 p-3 md:p-4 rounded-xl text-center font-bold tracking-widest focus:outline-none focus:ring-2 focus:ring-[#3E34FA] text-[#2B3674]" onKeyPress={(e) => e.key === 'Enter' && handleAdminLogin()} />
                               <button onClick={handleAdminLogin} className="w-full py-3 md:py-4 bg-[#3E34FA] text-white rounded-xl font-bold text-sm md:text-base shadow-md hover:bg-[#2B24C0] transition-all">Sisteme Giriş Yap</button>
                            </div>
                         </div>
                      </div>
                   ) : (
                      <div className="p-4 md:p-8 space-y-6 md:space-y-8 animate-in fade-in duration-500">
                         <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-5 md:p-6 rounded-[20px] md:rounded-[24px] shadow-sm border border-slate-100">
                            <div>
                               <h1 className="text-xl md:text-2xl font-bold text-[#2B3674]">Vaka Yönetim Paneli</h1>
                               <p className="text-xs md:text-sm text-[#A3AED0] font-bold mt-1">Sistemdeki vakaları ekleyin, düzenleyin veya silin.</p>
                            </div>
                            <button onClick={handleLogoutAdmin} className="w-full sm:w-auto px-6 py-3 bg-red-50 text-red-500 rounded-xl font-bold text-sm hover:bg-red-100 transition-all">Güvenli Çıkış</button>
                         </div>

                         <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 md:gap-8">
                            <form key={duzenlenenVaka ? duzenlenenVaka.vaka_adi : 'yeni_vaka'} onSubmit={handleVakaKaydet} className="col-span-1 bg-white p-6 md:p-8 rounded-[20px] md:rounded-[24px] shadow-sm border border-slate-100 space-y-4 h-fit">
                               <h3 className="text-base md:text-lg font-bold text-[#2B3674] mb-4 md:mb-6 flex items-center justify-between">
                               ...
                               <div><label className="text-[9px] md:text-[10px] font-bold text-[#A3AED0] uppercase tracking-widest block mb-2">Vaka Adı / Tanı</label>
                               ...
                               <div><label className="text-[9px] md:text-[10px] font-bold text-[#A3AED0] uppercase tracking-widest block mb-2">Vaka Özeti</label>
                               ...
                               <div><label className="text-[9px] md:text-[10px] font-bold text-[#A3AED0] uppercase tracking-widest block mb-2">Sistem Kuralları (Prompt)</label>
                               ...
                               <button type="submit" className={`w-full py-3 md:py-4 text-white rounded-xl font-bold text-sm shadow-md transition-all ${duzenlenenVaka ? 'bg-amber-500 hover:bg-amber-600' : 'bg-[#3E34FA] hover:bg-[#2B24C0]'}`}>
                                  {duzenlenenVaka ? "Vakayı Güncelle" : "Vakayı Veritabanına Kaydet"}
                               </button>
                            </form>

                            <div className="col-span-1 xl:col-span-2 bg-white p-6 md:p-8 rounded-[20px] md:rounded-[24px] shadow-sm border border-slate-100">
                               <h3 className="text-base md:text-lg font-bold text-[#2B3674] mb-4 md:mb-6 flex items-center gap-2"><span>🗂️</span> Kayıtlı Vakalar ({vakalar.length})</h3>
                               <div className="space-y-3 md:space-y-4">
                                  {vakalar.map((v, i) => (
                                     <div key={i} className={`flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 md:p-5 rounded-xl border transition-all gap-4 sm:gap-0 ${duzenlenenVaka?.vaka_adi === v.vaka_adi ? 'bg-amber-50 border-amber-300' : 'bg-[#F8F9FB] border-slate-100 hover:border-[#3E34FA]'}`}>
                                        <div className="flex items-center gap-3 md:gap-4">
                                           <div className={`w-8 h-8 md:w-10 md:h-10 rounded-lg flex items-center justify-center font-bold text-xs md:text-base ${duzenlenenVaka?.vaka_adi === v.vaka_adi ? 'bg-amber-200 text-amber-700' : 'bg-indigo-50 text-[#3E34FA]'}`}>V{i+1}</div>
                                           <div><h4 className="font-bold text-sm md:text-base text-[#2B3674]">{v.vaka_adi}</h4><p className="text-[9px] md:text-[10px] text-[#A3AED0] font-bold uppercase tracking-widest mt-1">Sistemde Kayıtlı</p></div>
                                        </div>
                                        <div className="flex flex-wrap gap-2 w-full sm:w-auto">
                                           <button onClick={() => setDuzenlenenVaka(v)} className="flex-1 sm:flex-none px-3 md:px-4 py-2 bg-amber-50 text-amber-600 rounded-lg text-xs font-bold hover:bg-amber-100 text-center">Düzenle</button>
                                           <button onClick={() => startTestSession(v)} className="flex-1 sm:flex-none px-3 md:px-4 py-2 bg-blue-50 text-blue-600 rounded-lg text-xs font-bold hover:bg-blue-100 text-center">Test Et</button>
                                           <button onClick={() => handleVakaSil(v.vaka_adi)} className="flex-1 sm:flex-none px-3 md:px-4 py-2 bg-red-50 text-red-500 rounded-lg text-xs font-bold hover:bg-red-100 text-center">Sil</button>
                                        </div>
                                     </div>
                                  ))}
             ...
             {activePage === 'seans-raporu' && (
               <div className="p-4 md:p-12 max-w-5xl mx-auto space-y-6 md:space-y-8 animate-in zoom-in-95 duration-500">
                 <div className="bg-[#3E34FA] rounded-[24px] md:rounded-[40px] p-6 md:p-10 text-white shadow-xl relative overflow-hidden">
                   <div className="relative z-10">
                     <h1 className="text-2xl md:text-3xl font-bold mb-1 md:mb-2">Simülasyon Tamamlandı</h1>
                     <p className="opacity-80 text-xs md:text-sm font-medium">Danışan: {currentVaka?.vaka_adi || 'Bilinmiyor'} | Toplam Süre: {formatTime(seansSuresi)}</p>
                   </div>
