...
export default function PsikoSimMaster() {
  ...
  const handleSeansBitir = async () => {
    ...
    try {
      ...
      setAnalizSonucu({
          empati_skoru: 0,
          terapotik_ittifak: "Seans analiz edilirken bir hata oluştu.",
          oneri: "Geri bildirim şu an oluşturulamadı, lütfen daha sonra tekrar deneyin."
      });
    } finally {
         ...
         <div className="flex w-full h-full bg-white relative">
            {chatPopup && (
                <div className="absolute inset-0 bg-white/95 z-40 flex flex-col p-6 md:p-12 animate-in fade-in">
                    <div className="flex justify-between items-center mb-8 border-b pb-4">
                        <h2 className="text-2xl md:text-3xl font-bold text-[#2B3674]">{chatPopup === 'dosya' ? 'Hasta Dosyası' : chatPopup === 'metrik' ? 'Klinik Metrikler' : 'Müdahaleler'}</h2>
                    ...
                    <div className="flex-1 overflow-y-auto text-base md:text-lg text-slate-600">
                        ...
                        {chatPopup === 'mudahale' && (
                            <ul className="list-none space-y-4">
                                <li className="bg-[#F8F9FB] p-6 rounded-2xl border border-slate-100"><b className="text-[#3E34FA] block mb-2">Yansıtıcı Dinleme</b> Danışanın son söylediği kelimeleri aynalayarak empatik bağ kurun.</li>
                                <li className="bg-[#F8F9FB] p-6 rounded-2xl border border-slate-100"><b className="text-[#3E34FA] block mb-2">Açık Uçlu Soru</b> "Bu hissettiğin duyguyu biraz daha açabilir misin?"</li>
                            </ul>
                        )}
                    </div>
                </div>
            ...
            <aside className="hidden lg:flex w-[280px] bg-[#F8F9FB] border-r border-slate-200 flex flex-col p-6 shrink-0 z-10">
               ...
               <nav className="flex-1 space-y-2">
                  <button className="w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl text-sm font-bold bg-white text-[#3E34FA] shadow-sm border border-slate-100"><span className="text-lg">⚙️</span> Session Hub</button>
                  <button onClick={() => setChatPopup('dosya')} className="w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl text-sm font-bold text-[#A3AED0] hover:bg-white/50 transition-all"><span className="text-lg">👤</span> Hasta Dosyası</button>
                  <button onClick={() => setChatPopup('metrik')} className="w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl text-sm font-bold text-[#A3AED0] hover:bg-white/50 transition-all"><span className="text-lg">📊</span> Metrikler</button>
                  <button onClick={() => setChatPopup('mudahale')} className="w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl text-sm font-bold text-[#A3AED0] hover:bg-white/50 transition-all"><span className="text-lg">✨</span> Müdahaleler</button>
               </nav>
               <button onClick={handleSeansBitir} className="w-full py-4 bg-[#B91C1C] text-white rounded-xl font-bold text-sm shadow-md flex justify-center items-center gap-2 hover:bg-red-800 transition-all">↪ Seansı Bitir</button>
            </aside>
            ...
            <section className="flex-1 flex flex-col bg-[#F8FAFC] relative border-r border-slate-200">
               ...
               <header className="h-16 md:h-20 px-4 md:px-8 flex items-center justify-between border-b border-slate-100 bg-white/50 backdrop-blur-md z-10 shrink-0">
                  <div className="flex items-center gap-2 bg-[#E9E3FF] px-3 md:px-4 py-1.5 rounded-full"><div className="w-2 h-2 bg-[#3E34FA] rounded-full animate-pulse"></div><span className="text-[9px] md:text-[10px] font-bold text-[#3E34FA] uppercase tracking-widest">{isTestMode ? "TEST" : "AKTİF SEANS"}</span></div>
                  <button onClick={handleSeansBitir} className="lg:hidden px-3 py-1.5 bg-red-50 text-red-600 rounded-lg font-bold text-[10px] uppercase tracking-widest border border-red-100 shadow-sm">Bitir ↪</button>
                  <div className="hidden md:flex items-center gap-8">
                     <div className="text-center"><p className="text-[10px] font-bold text-[#A3AED0] uppercase tracking-widest">SEANS SÜRESİ</p><p className="text-xl font-black text-[#2B3674] leading-none mt-1">{formatTime(seansSuresi)}</p></div>
                     <div className="text-center"><p className="text-[10px] font-bold text-[#A3AED0] uppercase tracking-widest">HASTA DURUMU</p><p className="text-sm font-bold text-[#3E34FA] leading-none mt-1">Klinik İletişim</p></div>
                  </div>
               </header>
                  ...
                  <div className="max-w-3xl mx-auto space-y-4 md:space-y-6">
                     {mesajlar.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-64 text-center opacity-60 mt-10 md:mt-20">
                           ...
                           <p className="text-xs md:text-sm text-[#A3AED0] px-4">İlk sorunuzu yazabilir veya sesli iletebilirsiniz.</p>
                        </div>
                     ) : (
                        mesajlar.map((m, i) => (
                           <div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
                              ...
                              <p className="text-[9px] md:text-[10px] font-bold text-[#A3AED0] uppercase mt-2 tracking-widest">{m.role === 'user' ? (userName ? userName.split(' ')[0] : 'SEN') : 'DANIŞAN'} • {m.time}</p>
                           </div>
                        ))
                     )}
                     <div ref={scrollRef} />
            ...
            <aside className="hidden xl:flex w-[360px] bg-[#F8F9FB] p-6 flex-col gap-6 overflow-y-auto shrink-0 z-10 border-l border-slate-200">
               <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 border-t-4 border-t-[#3E34FA]">
                  <div className="flex justify-between items-center mb-6"><h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest">STRES SEVİYESİ</h4></div>
      ...
      <>
        ...
        <aside className={`fixed lg:static inset-y-0 left-0 transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-300 ease-in-out w-[260px] bg-[#F8FAFC] border-r border-slate-200 flex flex-col shrink-0 z-50`}>
          ...
          <nav className="flex-1 flex flex-col px-4 space-y-1 mt-2">
            ...
            <div className="mt-auto pt-6 lg:pt-10 pb-2"><button onClick={() => setActivePage('admin')} className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-full text-[14px] font-semibold transition-all ${activePage === 'admin' ? 'bg-slate-200 text-[#1E293B] shadow-sm' : 'text-[#64748B] hover:bg-slate-100 hover:text-[#1E293B]'}`}><svg className="w-5 h-5 opacity-90" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>Yetkili Paneli</button></div>
          </nav>
          <div className="p-4 lg:p-6 space-y-4"><button onClick={() => setActivePage('library')} className="w-full py-3.5 bg-[#3E34FA] text-white rounded-full font-bold text-sm shadow-md hover:bg-[#2B24C0] transition-all flex items-center justify-center gap-2"><span className="text-lg">+</span> Yeni Simülasyon</button></div>
        </aside>
           ...
           <div className="flex-1 overflow-y-auto">
               ...
               <div className="flex flex-col lg:flex-row h-full animate-in fade-in duration-300">
                 <div className="flex-1 p-4 md:p-8 space-y-6 md:space-y-8">
                   <div className="bg-gradient-to-r from-[#2111E7] to-[#4537F3] rounded-[24px] md:rounded-[32px] p-8 md:p-12 text-white shadow-xl relative overflow-hidden">
                     <div className="relative z-10 max-w-2xl"><h1 className="text-2xl md:text-4xl font-bold mb-3 md:mb-4 leading-snug">Psikolog Adayları İçin Geliştirilmiş Sanal Danışan Simülasyonu</h1><p className="text-indigo-100 text-sm md:text-base mb-6 md:mb-8 opacity-90">Teorik bilginizi pratikle buluşturun, güvenli bir ortamda gerçek vaka deneyimleri yaşayın.</p><button onClick={() => setActivePage('library')} className="w-full md:w-auto bg-white text-[#3E34FA] px-6 md:px-8 py-3.5 rounded-full font-bold text-sm shadow-lg flex items-center justify-center gap-2 hover:scale-105 transition-all">▶ Vaka Seç ve Başla</button></div>
                   </div>
                   <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                     {[
                       { t: 'SİSTEM DURUMU', v: 'Çevrimiçi', d: 'Bağlantı Hazır', icon: '🟢', c: 'text-emerald-500', bc: 'border-emerald-400' },
                       { t: 'KAYITLI VAKA', v: isLoading ? 'Yükleniyor...' : `${filteredVakalar.length} Danışan`, d: 'Seans Bekliyor', icon: '👤', c: 'text-[#3E34FA]', bc: 'border-[#3E34FA]' },
                   ...
                   <div className="bg-white rounded-[24px] md:rounded-[32px] p-6 md:p-8 shadow-sm border border-slate-100 relative overflow-hidden group transition-all hover:shadow-md">
                     <div className="flex justify-between items-center mb-6"><div><h3 className="text-base md:text-lg font-bold text-[#1E293B]">Aktif Simülasyon</h3><p className="text-xs text-[#64748B] font-medium">Devam eden seans verileri</p></div><button onClick={() => setActivePage('library')} className="text-[#3E34FA] text-xs font-bold hover:underline">Tümünü Gör ➔</button></div>
                     <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-8 bg-[#F8FAFC] p-4 md:p-6 rounded-2xl md:rounded-3xl border border-slate-50">
                       ...
                       <div className="flex-1 w-full"><div className="flex items-center gap-3 mb-1"><h4 className="text-sm md:text-base font-bold text-[#1E293B]">{currentVaka?.vaka_adi || "Seçili Vaka Yok"}</h4><span className="bg-[#E9E3FF] text-[#3E34FA] text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-tighter">DEVAM EDİYOR</span></div><p className="text-[11px] md:text-xs text-[#64748B] font-medium">Son etkileşim: {mesajlar.length > 0 ? mesajlar[mesajlar.length - 1].time : "Henüz başlanmadı"}</p></div>
                       <div className="flex items-center gap-6 md:gap-10 sm:pr-4 w-full sm:w-auto justify-between sm:justify-end border-t sm:border-t-0 pt-4 sm:pt-0"><div className="text-center"><p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest">MESAJLAR</p><p className="text-base md:text-lg font-bold text-[#1E293B]">{mesajlar.length}</p></div><div className="text-center"><p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest">SÜRE</p><p className="text-base md:text-lg font-bold text-[#1E293B]">{formatTime(seansSuresi)}</p></div><button onClick={() => { if(currentVaka) setActivePage('chat-session') }} className="w-10 h-10 bg-white border border-slate-200 rounded-full flex items-center justify-center shadow-sm hover:bg-[#3E34FA] hover:text-white transition-all sm:group-hover:scale-110">➔</button></div>
                     </div>
                   </div>
                 </div>
                 <div className="w-full lg:w-[340px] bg-white border-t lg:border-t-0 lg:border-l border-slate-200 p-6 md:p-8 flex flex-col items-center shrink-0">
                    <div className="w-full flex justify-between items-center mb-6 md:mb-8"><span className="text-[10px] font-bold text-[#A3AED0] uppercase tracking-widest">DANIŞAN DOSYASI</span><span className="text-slate-300">🔍</span></div>
                    ...
                    <h3 className="text-xl md:text-2xl font-bold text-[#2B3674] line-clamp-1">{isLoading ? "Yükleniyor..." : (filteredVakalar[0]?.vaka_adi || "Vaka Bulunamadı")}</h3>
             ...
             {activePage === 'library' && (
               <div className="p-4 md:p-8 space-y-6 md:space-y-8 animate-in fade-in duration-300">
                  <div className="max-w-3xl"><h1 className="text-2xl md:text-3xl font-bold text-[#2B3674] mb-3 md:mb-4">Simülasyon Vakaları</h1><p className="text-sm md:text-base text-[#A3AED0] font-medium leading-relaxed">Klinik deneyiminizi geliştirmek için titizlikle hazırlanmış gerçekçi vaka senaryolarını keşfedin.</p></div>
                ...
                <div className="flex-1 flex flex-col h-full animate-in fade-in duration-300">
                   {!isAdminAuth ? (
                      <div className="flex-1 flex items-center justify-center p-4 md:p-8">
                         <div className="bg-white p-8 md:p-12 rounded-[24px] md:rounded-[32px] shadow-xl max-w-md w-full text-center relative overflow-hidden"><div className="absolute top-0 left-0 w-full h-2 bg-[#3E34FA]"></div><div className="w-16 h-16 md:w-20 md:h-20 bg-[#F4F7FE] text-[#3E34FA] rounded-[20px] flex items-center justify-center text-2xl mx-auto mb-6 shadow-inner">🔒</div><h2 className="text-xl md:text-2xl font-bold text-[#2B3674] mb-2">Yetkili Girişi</h2><input type="password" value={adminPassword} onChange={(e) => setAdminPassword(e.target.value)} placeholder="Şifre" className="w-full bg-[#F8F9FB] border border-slate-200 p-3 md:p-4 rounded-xl text-center font-bold mb-6 focus:outline-none focus:ring-2 focus:ring-[#3E34FA]" onKeyPress={(e) => e.key === 'Enter' && handleAdminLogin()} /><button onClick={handleAdminLogin} className="w-full py-3 md:py-4 bg-[#3E34FA] text-white rounded-xl font-bold shadow-md hover:bg-[#2B24C0] transition-all">Giriş Yap</button></div>
                      </div>
                   ) : (
                      <div className="p-4 md:p-8 space-y-6 md:space-y-8">
                         <div className="flex flex-col sm:flex-row justify-between items-center bg-white p-5 rounded-[20px] shadow-sm border border-slate-100 w-full"><div><h1 className="text-xl md:text-2xl font-bold text-[#2B3674]">Vaka Yönetimi</h1></div><button onClick={handleLogoutAdmin} className="px-6 py-2 bg-red-50 text-red-500 rounded-xl font-bold text-sm">Çıkış</button></div>
                         <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 md:gap-8">
                            <form key={duzenlenenVaka ? duzenlenenVaka.vaka_adi : 'yeni'} onSubmit={handleVakaKaydet} className="bg-white p-6 md:p-8 rounded-[20px] shadow-sm border border-slate-100 space-y-4">
                               <h3 className="text-lg font-bold text-[#2B3674] mb-4">{duzenlenenVaka ? "🔄 Güncelle" : "📝 Ekle"}</h3>
                               <input name="vaka_adi" defaultValue={duzenlenenVaka?.vaka_adi} placeholder="Vaka Adı" className="w-full bg-[#F8F9FB] border border-slate-200 p-3 rounded-lg text-sm" required />
                               <textarea name="ozet" defaultValue={duzenlenenVaka?.ozet} placeholder="Özet" rows={3} className="w-full bg-[#F8F9FB] border border-slate-200 p-3 rounded-lg text-sm" required />
                               <textarea name="kurallar" defaultValue={duzenlenenVaka?.kurallar} placeholder="Prompt" rows={5} className="w-full bg-[#F8F9FB] border border-slate-200 p-3 rounded-lg text-sm" required />
                               <button type="submit" className="w-full py-3 bg-[#3E34FA] text-white rounded-xl font-bold text-sm">{duzenlenenVaka ? "Güncelle" : "Kaydet"}</button>
                            </form>
                            <div className="xl:col-span-2 bg-white p-6 md:p-8 rounded-[20px] shadow-sm border border-slate-100 space-y-3">{vakalar.map((v, i) => (<div key={i} className="flex flex-col sm:flex-row justify-between items-center p-4 bg-[#F8F9FB] rounded-xl border border-slate-100 gap-4"><h4 className="font-bold text-[#2B3674]">{v.vaka_adi}</h4><div className="flex gap-2"><button onClick={() => setDuzenlenenVaka(v)} className="px-4 py-2 bg-amber-50 text-amber-600 rounded-lg text-xs font-bold">Düzenle</button><button onClick={() => startTestSession(v)} className="px-4 py-2 bg-blue-50 text-blue-600 rounded-lg text-xs font-bold">Test Et</button><button onClick={() => handleVakaSil(v.vaka_adi)} className="px-4 py-2 bg-red-50 text-red-500 rounded-lg text-xs font-bold">Sil</button></div></div>))}</div>
                         </div>
                      </div>
                   )}
                </div>
             ...
             {activePage === 'seans-raporu' && (
               <div className="p-4 md:p-12 max-w-5xl mx-auto space-y-6 md:space-y-8 animate-in zoom-in-95 duration-500">
                 <div className="bg-[#3E34FA] rounded-[24px] md:rounded-[40px] p-6 md:p-10 text-white shadow-xl relative overflow-hidden">
                   <div className="relative z-10"><h1 className="text-2xl md:text-3xl font-bold mb-1 md:mb-2">Simülasyon Tamamlandı</h1><p className="opacity-80 text-xs md:text-sm font-medium">Danışan: {currentVaka?.vaka_adi || 'Bilinmiyor'}</p></div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                   <div className="col-span-1 md:col-span-2 space-y-6">
                     <div className="bg-white rounded-[24px] md:rounded-[32px] p-6 md:p-8 border border-slate-100 shadow-sm min-h-[250px]">
                       ...
                       {isAnalyzing ? (<div className="flex flex-col items-center justify-center h-40 space-y-4 text-center"><div className="w-10 h-10 border-4 border-[#3E34FA] border-t-transparent rounded-full animate-spin"></div><p className="text-[#3E34FA] font-bold text-sm animate-pulse">Yapay Zeka Seansı Analiz Ediyor...</p></div>) : (<div className="space-y-4 md:space-y-6 text-slate-600 text-xs md:text-sm leading-relaxed"><p><b className="text-[#3E34FA]">Terapötik İttifak:</b> {analizSonucu?.terapotik_ittifak}</p><div className="bg-slate-50 p-4 md:p-6 rounded-xl md:rounded-2xl border-l-4 border-[#3E34FA]"><p className="font-bold text-[#1E293B] mb-1 md:mb-2">Öneri:</p>{analizSonucu?.oneri}</div></div>)}
                     </div>
                   </div>
                   <div className="space-y-6">
